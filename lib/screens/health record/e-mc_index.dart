...
class EMCScreen extends StatefulWidget {
  final String orderId;
  const EMCScreen({Key? key, required this.orderId}) : super(key: key);

  @override
  State<EMCScreen> createState() => _EMCScreenState();
}

class _EMCScreenState extends State<EMCScreen> {
  Future? futureFetchPanelEmc;
  int selected = -1;
  @override
  void initState() {
    super.initState();
    futureFetchPanelEmc = ApiService().fetchPanelRecords(widget.orderId);
  }
                  ...
                  FutureBuilder(
                      ...
                      builder: (context, AsyncSnapshot snapshot) {
                        if (snapshot.connectionState ==
                          ...
                          if (snapshot.hasData) {
                            List eMcList =
                                snapshot.data['medical_certificates'];
                            print('hasData');
                            print(eMcList);
                            return (eMcList.isNotEmpty)
                                ? ListView.separated(
                                    ...
                                    itemCount: snapshot
                                        .data['medical_certificates'].length,
                                    itemBuilder: (context, index) {
